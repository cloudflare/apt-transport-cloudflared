...
func NewMessage(statusCode uint64, description string, fields map[string]string) *Message {
    return &Message{
        statusCode,
        description,
        fields,
...
func (r *MessageReader) ReadLine() (*Message, error) {
    ...
    if err != nil {
        return r.commitMessage(), err
    }

    line = strings.TrimSpace(line)
    if line == "" {
        return r.commitMessage(), nil
    }

    parts := strings.SplitN(line, ":", 2)
    if len(parts) != 2 {
        // We didn't get an optionA
        return r.commitMessage(), fmt.Errorf("Invalid field format in \"%s\"", line)
    }
...
func (r *MessageReader) readHeader() (*Message, error) {
    ...
    parts := strings.SplitN(line, " ", 2)
    if len(parts) != 2 {
        return nil, fmt.Errorf("Invalid header format in \"%s\"", line)
    }

    // Attempt to parse the status code (parts[0])
    codeStr := strings.TrimSpace(parts[0])
    code, err := strconv.ParseUint(codeStr, 10, 64)
    if err != nil {
        return nil, fmt.Errorf("Could not parse status code from \"%s\"", line)
    }

    desc := strings.TrimSpace(parts[1])
    if desc == "" {
        return nil, fmt.Errorf("Empty description in header for \"%s\"", line)
    }
...
func (r *MessageReader) commitMessage() *Message {
    msg := r.message
    r.message = nil
    return msg
}
